...
typedef struct {
    ...
    int type;       // 消息类型
    union {
        struct { int u1, u2, u3, u4; };           // 通用整型载荷
        struct { void* ptr; int _p1, _p2, _p3; }; // 指针载荷
        struct { int val; int _i1, _i2, _i3; } integer; // 别名方便使用
    };
} message_t;
 ...
 * \param src_dest  如果是 SEND，表示目标 PID；如果是 RECEIVE，表示源 PID (或 ANY)
